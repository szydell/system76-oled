...
#
# ----------------------------------------------------------------------------
# 2026-09-21 Marcin Szydelski
#   changes for system76-oled
#
...
version_in_changelog=$(grep -E "system76-oled \([[:digit:]]+\.[[:digit:]]+\.[[:digit:]]+" debian/changelog | head -1)
_tmp=${version_in_changelog%%)*}
_tmp=${_tmp%%~*}
version=${_tmp##*\(}

_tmp=$(git tag --list system76-oled-"$version"-'*' | sort -n -t '-' -k 4 -r | head -1)
release=${_tmp##*-}
...
sed -i "s#^Version:    .*#Version:    $version#" system76-oled.spec.rpkg
sed -i "s#^Release:    .*#Release:    $release#" system76-oled.spec.rpkg
git commit -m"bump Version to: $version-$release" system76-oled.spec.rpkg
...
srpm="$(ls .rpkg-build/system76-oled-*.src.rpm)"
